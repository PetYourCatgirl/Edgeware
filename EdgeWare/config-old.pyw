...
def configWindow(delay, fill, replace, webMod, popupMod, audioMod, promptMod, slow):
    ...
    popupSlider = Scale(
        ...
        to=100,
        orient=HORIZONTAL,
        variable=popupModVar,
    )
    popupSlider.set(popupMod)
    popupSlider.pack()
    audioSlider = Scale(
        root, label="Audio Freq", from_=0, to=100, orient=HORIZONTAL, variable=audModVar
    )
...
configWindow(
    int(settingJsonObj["delay"]),
    int(settingJsonObj["fill"]) == 1,
    int(settingJsonObj["replace"]) == 1,
    int(settingJsonObj["webMod"]),
    int(settingJsonObj["popupMod"]),
    int(settingJsonObj["audioMod"]),
    int(settingJsonObj["promptMod"]),
    int(settingJsonObj["slowMode"]) == 1,
)
